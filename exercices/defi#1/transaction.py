...
def decodeTransaction(transaction_bytearray):
    ...
    version = littleEndianToInt(transaction_bytearray[0:VERSION_FIELD_LEN]);
    ...
    locktime = hex_to_str(transaction_bytearray[len_varint_outputs + start_index : len_varint_outputs + start_index + LOCKTIME_FIELD_LEN]);
    transaction_decode = Transaction(version, inputs, outputs, locktime);
    transaction_decode.length = len_varint_outputs + start_index + LOCKTIME_FIELD_LEN;
